...
This chart follows the same provider pattern as the DocumentDB and OpenSearch charts, but defaults to no cache AUTH so it matches existing non-auth ElastiCache clusters:
...
- by default `auth.mode=disabled`, so `ReplicationGroup.spec.authToken` is omitted and the connection secret gets an empty password key
- if you set `auth.mode=password`, it generates a cache password and points `ReplicationGroup.spec.authToken` at that generated password key
- if you set `auth.mode=secretRef`, it points ACK at an existing secret reference instead
- if you set `sequencedConnection.enabled=true`, Argo hook jobs create a stable connection secret before sync and patch `HOST`, `PORT`, and `ARN` into it after ACK reports the primary endpoint
- default connection secret keys use the engine-agnostic `CACHE_*` prefix for both Redis OSS and Valkey
...
| auth | object | `{"existingSecret":{"key":"","name":"","namespace":""},"mode":"disabled","password":{"allowRepeat":true,"digits":8,"key":"CACHE_PASSWORD","length":32,"noUpper":false,"symbols":4},"username":"default"}` | Authentication data written into the generated connection secret and used for `spec.authToken`. |
...
| auth.mode | string | `"disabled"` | Disable AUTH by default, or use a generated cache password or existing secret reference. |
...
| auth.password.key | string | `"CACHE_PASSWORD"` | Key used for the generated password in the connection secret and `spec.authToken`. |
...
| cacheParameterGroup | object | `{"create":false,"description":"","family":"","name":"","parameters":{}}` | Optional CacheParameterGroup to create and attach to the replication group. |
| cacheParameterGroup.create | bool | `false` | Create a CacheParameterGroup resource. |
| cacheParameterGroup.description | string | `""` | Optional description for the cache parameter group. |
| cacheParameterGroup.family | string | `""` | Cache parameter group family, for example `redis7`. |
| cacheParameterGroup.name | string | `""` | AWS CacheParameterGroup name. Defaults to resourceName/fullname when omitted. |
| cacheParameterGroup.parameters | object | `{}` | Parameter name/value map rendered into `parameterNameValues`. |
| cacheSubnetGroupName | string | `nil` | Cache subnet group name from `ReplicationGroup.spec.cacheSubnetGroupName`. |
| connectionSecret | object | `{"annotations":{},"create":true,"keys":{"arn":"CACHE_ARN","authURL":"CACHE_AUTH_URL","host":"CACHE_HOST","port":"CACHE_PORT","tls":"CACHE_TLS","url":"CACHE_URL","username":"CACHE_USERNAME"},"name":""}` | Settings for the generated in-cluster connection secret. |
...
| connectionSecret.keys | object | `{"arn":"CACHE_ARN","authURL":"CACHE_AUTH_URL","host":"CACHE_HOST","port":"CACHE_PORT","tls":"CACHE_TLS","url":"CACHE_URL","username":"CACHE_USERNAME"}` | Output key names written into the generated connection secret. |
...
| fieldExport | object | `{"enabled":true,"mappings":[{"key":"CACHE_HOST","name":"host","path":".status.nodeGroups.0.primaryEndpoint.address"},{"key":"CACHE_PORT","name":"port","path":".status.nodeGroups.0.primaryEndpoint.port"},{"key":"CACHE_ARN","name":"arn","path":".status.ackResourceMetadata.arn"}]}` | FieldExport settings for copying ReplicationGroup status fields into the connection secret. |
| fieldExport.mappings | list | `[{"key":"CACHE_HOST","name":"host","path":".status.nodeGroups.0.primaryEndpoint.address"},{"key":"CACHE_PORT","name":"port","path":".status.nodeGroups.0.primaryEndpoint.port"},{"key":"CACHE_ARN","name":"arn","path":".status.ackResourceMetadata.arn"}]` | Fields exported from ReplicationGroup status into the generated secret. |
...
| pushSecret | object | `{"enabled":false,"name":"","refreshInterval":"1h","sourceKey":"CACHE_HOST","storeKind":"SecretStore","target":{"name":"","provider":"","type":"Opaque"}}` | External Secrets PushSecret settings for publishing a selected secret key to an external store. |
...
| snapshotName | string | `nil` | Existing ElastiCache snapshot name used to seed the new replication group. |
