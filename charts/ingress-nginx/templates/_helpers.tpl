...
{{/*
Resolve an image registry with a global override and a template-specific
fallback.
*/}}
{{- define "ingress-nginx.imageRegistry" -}}
{{- if .image.registry -}}
{{- .image.registry -}}
{{- else if .global.registry -}}
{{- .global.registry -}}
{{- else -}}
{{- .defaultRegistry -}}
{{- end -}}
...
- name: {{ .name }}
  {{- with .image }}
  {{- $registry := include "ingress-nginx.imageRegistry" (dict "image" . "global" $.globalImage "defaultRegistry" $.defaultRegistry) -}}
