...
def test_legacy_dbcluster_values_still_render(helm_runner) -> None:
    """Ensure the prior nested dbCluster values shape remains supported."""
    ...
    assert cluster["metadata"]["name"] == "legacy-resource"
    assert cluster["metadata"]["annotations"]["example.com/test"] == "true"
    assert cluster["spec"]["dbClusterIdentifier"] == "legacy-docdb"
