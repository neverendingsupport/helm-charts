...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "requires at least one effective metric",
...
def test_autoscaling_rejects_cross_field_misconfiguration(
    helm_runner,
    values: dict[str, Any],
    message: str,
) -> None:
...
def test_legacy_utilization_renders_without_declared_requests(
    ...
    """Keep existing percentage-based HPA releases renderable."""
...
def test_autoscaling_accepts_request_from_extra_container_props(
    helm_runner,
) -> None:
...
def test_autoscaling_uses_extra_container_resource_precedence(
    helm_runner,
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            id="autoscaling-annotation-is-not-string",
        ),
        pytest.param(
            {
                "autoscaling": {
            ...
            id="autoscaling-behavior-period-out-of-range",
        ),
        pytest.param(
            {
                "autoscaling": {
        ...
        pytest.param(
            {"autoscaling": {"horizontal": {"enabled": True}}},
            id="unreleased-horizontal-interface-is-rejected",
        ),
    ],
)
